...
def classify_columns(headers):
    """Map column headers to roles: date, description, withdrawal, deposit, balance."""
    mapping = {}
...
def extract_with_pdfplumber_tables(pdf):
    """Primary method: use pdfplumber's built-in table detection."""
    all_rows = []

    # Try strategies in order of reliability
    strategies = [
        # 1. Line-based (works when PDF has drawn table borders)
        {"vertical_strategy": "lines", "horizontal_strategy": "lines"},
        # 2. Strict lines
        {"vertical_strategy": "lines_strict", "horizontal_strategy": "lines_strict"},
        # 3. Text-based with tuned tolerances
        {
    ...
    for strategy in strategies:
        ...
        # Check if we got meaningful data (header + at least a few rows)
        if len(all_rows) >= 3:
            # Quick quality check: do rows have consistent column counts?
            col_counts = [len(r) for r in all_rows]
            most_common = max(set(col_counts), key=col_counts.count)
            good_rows = sum(1 for c in col_counts if c == most_common)
            if good_rows >= len(all_rows) * 0.5:
                return all_rows

    return all_rows


def extract_with_words(pdf):
    """Fallback: extract words with positions and reconstruct rows/columns."""
    all_words = []
    for page in pdf.pages:
    ...
    all_words.sort(key=lambda w: (w['page'], w['top'], w['x0']))
    rows_grouped = []
    current_row = [all_words[0]]
    for w in all_words[1:]:
        if w['page'] != current_row[0]['page'] or abs(w['top'] - current_row[0]['top']) > 4:
    ...
    # Find column boundaries from x-positions using clustering
    x_positions = [w['x0'] for w in all_words]
    x_positions.sort()
    ...
    for x in x_positions:
        b = round(x / bucket_size) * bucket_size
        buckets[b] = buckets.get(b, 0) + 1

    # Find column starts (prominent x clusters)
    min_count = len(rows_grouped) * 0.08
    col_starts = sorted([x for x, c in buckets.items() if c >= min_count])
    ...
    if len(col_starts) < 2:
        # Just return raw text rows
        return [[' '.join(w['text'] for w in row)] for row in rows_grouped]
...
def parse_pdf(file_bytes):
    """Parse a bank statement PDF and return withdrawals + deposits."""
    pdf = pdfplumber.open(io.BytesIO(file_bytes))

    # Try table extraction first
    rows = extract_with_pdfplumber_tables(pdf)

    method = 'tables'
    if len(rows) < 3:
        # Fallback to word-based extraction
        rows = extract_with_words(pdf)
        method = 'words'

    if not rows:
        return {'withdrawals': [], 'deposits': [], 'raw_rows': [], 'method': method,
                'error': 'No text found. The PDF may be a scanned image.'}
    ...
    for i, row in enumerate(rows[:30]):
        ...
        score = 0
        for pat in HEADER_KEYWORDS.values():
            if pat.search(combined):
                score += 1
        if score > best_score:
            best_score = score
            header_idx = i
            col_mapping = classify_columns(row)

    # If no good header found, use heuristic column assignment
    if best_score < 2:
        header_idx = -1
    ...
    # Detected column headers for the UI
    detected_headers = []
    if header_idx >= 0 and header_idx < len(rows):
        detected_headers = [c.strip() for c in rows[header_idx]]
    else:
        num_cols = max(len(r) for r in rows) if rows else 0
        detected_headers = [f'Column {i+1}' for i in range(num_cols)]
    ...
    for row in data_rows:
        ...
        while len(row) < len(detected_headers):
            row.append('')
        ...
        # No amounts — could be a continuation line
        if wd_amount is None and dp_amount is None:
            if desc_val:
                # Append to most recent transaction
                target = None
                if withdrawals and deposits:
                    # Pick the one added most recently (by list length proxy — not perfect but works)
    ...
    # Fallback: if only one amount column mapped and nothing found, try treating
    # negative as withdrawals and positive as deposits from any numeric column
    if not withdrawals and not deposits and 'withdrawal' in col_mapping:
        for row in data_rows:
            while len(row) < len(detected_headers):
                row.append('')
    ...
    # Return raw rows for debugging / manual column reassignment
    raw_sample = rows[:min(len(rows), 5)]
...
<p class="privacy-note">&#128274; Your PDF is processed locally on your computer using pdfplumber. Nothing is uploaded to the internet.</p>

<script>
let lastResult = null;  // store last parse result for re-mapping
...
async function uploadFile(file) {
  showStatus('Parsing PDF with pdfplumber...', 'info');
  try {
...
async function reparse() {
  // Read user's column mapping and re-parse server-side
...
function renderResults(result) {
  ...
  if (wCount + dCount === 0) {
    ...
    showStatus(`Found ${wCount} withdrawal(s) and ${dCount} deposit(s). Parsed ${result.total_rows} rows via ${result.method} method.`, 'success');
  }
...
class Handler(http.server.BaseHTTPRequestHandler):
    last_pdf_bytes = None  # class-level storage for re-parse
    ...
    def do_POST(self):
        if self.path == '/parse':
            ...
            if Handler.last_pdf_bytes is None:
                self._json_response({'error': 'No PDF loaded. Please upload a PDF first.'})
                return
...
def reparse_with_mapping(pdf_bytes, col_mapping):
    """Re-parse the stored PDF with a user-specified column mapping."""
    ...
    if not rows:
        return {'withdrawals': [], 'deposits': [], 'error': 'No text found.'}

    # Find and skip header row
    header_idx = -1
    for i, row in enumerate(rows[:30]):
        combined = ' '.join(c.strip() for c in row if c.strip())
        score = sum(1 for pat in HEADER_KEYWORDS.values() if pat.search(combined))
        if score >= 2:
            header_idx = i
            break
...
def main():
    ...
    print(f"Bank Statement Parser running at http://127.0.0.1:{PORT}")
